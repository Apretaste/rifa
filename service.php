		...
		if ($id !== null) {
			...
			if ($raffle === null) {
				return $response->setTemplate('message.ejs', [
					'header' => 'No hay rifas abiertas',
					'icon' => 'sentiment_very_dissatisfied',
					'text' => 'Lo sentimos, esa rifa no existe.',
					'button' => ['href' => 'RIFA', 'caption' => 'Rifa actual']
				]);
			}

			// get number of tickets adquired by the user
			$tickets = Database::queryFirst("SELECT COUNT(ticket_id) AS cnt FROM ticket WHERE raffle_id = $id AND person_id = '{$request->person->id}'");
			$raffle->tickets = (int) $tickets->cnt;
